node {
    withCredentials([[$class: 'UsernamePasswordMultiBinding',
        ...
        stage('Pull') {
            try {
                git branch: 'main', url: 'https://github.com/Gyemoo/fisa05-jenkins-practice.git'
        ...
        stage('Deploy') {
            sshagent(credentials: ['ec2-fastapi-server']) {
                ...
                if (env.BUILD_NUMBER.toInteger() != 1) {
                    sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@52.79.193.78 "sudo docker rm -f docker-fastapi"')
                } else {
                    echo "Initial run (BUILD_NUMBER=${env.BUILD_NUMBER}). Skipping docker container removal."
                }
                // sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@52.79.193.78 "pwd"')
                sh(script: 'ssh -o StrictHostKeyChecking=no ubuntu@52.79.193.78 "sudo docker run --name docker-fastapi --env-file .env -e TZ=Asia/Seoul -p 80:8000 -d -t ${DOCKER_USER_ID}/fisa-app:${BUILD_NUMBER}"')
            }
